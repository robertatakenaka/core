...
class EditorialBoardMember(CommonControlField, ClusterableModel):
    ...
    def _create(
        ...
        if journal and researcher:
            try:
                ...
                return cls.get(researcher, journal)
    ...
    def create_or_update(
        ...
        if not researcher:
            ...
            country = None
            if state_text:
                state = State.standardize(state_text, user)
            if country_text:
                country = Country.standardize(country_text, user)
            ...
            try:
                location = Location.create_or_update(
                    ...
                    country_acron3=None,
                    country_acronym=institution_country_acronym,
                    state=state,
                    state_name=institution_state_name,
                    state_acronym=institution_state_acronym,
                    city=institution_city_name,
                    city_name=institution_city_name,
                    lang=None,
                )
            ...
            gender = Gender.get_or_create(user, code=gender_code, gender=gender_code)
        ...
        if researcher and journal:
            member = cls._create_or_update(
                user,
                journal,
                researcher,
            )

        if member and (std_role or declared_role) and year:
    ...
    def create_or_update(
        ...
        if member and initial_year and role and initial_month:
            try:
                ...
                return cls._create(user, member, role, initial_year, initial_month)
        raise ValueError(
