...
class PidProviderXMLAdapterContribGroupTest(TestCase):
    ...
    def test_surnames(self):
        self.assertEqual(
            _str_with_64_char("Torquato|Santis|Zanetti"),
            self.xml_adapter.surnames)


class PidProviderXMLAdapterArticleTitlesTest(TestCase):
    def _get_xml_adapter(self, main_title=None, trans_titles=None, sub_article_titles=None):
    ...
    def test_article_titles_texts_en_pt_es(self):
        xml_adapter = self._get_xml_adapter(main_title=True, trans_titles=True, sub_article_titles=True)
        self.assertEqual(
            _str_with_64_char("Article title in English|Título em português|título en español"),
            xml_adapter.article_titles_texts)
...
class PidProviderXMLAdapterValidateQueryParamsTest(TestCase):
    ...
    def test_validate_query_params_all_present(self):
        ...
        result = self.xml_adapter.validate_query_params(params)
