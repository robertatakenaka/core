...
def get_mongodb_collection(mongodb_collection_name):
    ...
    """
    try:
        return get_client()[mongodb_collection]
    except Exception as e:
        raise Exception(f"Failed to connect to MongoDB collection: {mongodb_collection}: {str(e)}")
...
def bulk_write(mongodb_collection_name, items_to_upsert, max_per_bulk=None):
    ...
    # Executando todas as operações em lote
        try:
            ...
            exceptions.append(
                f"Ocorreu um erro na operação em lote: {e}"
            )
